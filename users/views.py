...
from .permissions import IsAdmin, IsTechnician, IsClient, IsAdminOrTechnician, IsAdminOrOwner, IsOwnerOrAdmin
User = get_user_model()
...
class UserUpdateByIdView(generics.RetrieveUpdateAPIView):
    permission_classes = [IsAdmin]  # <- Habilitado solo para pruebas
    serializer_class = UserUpdateSerializer
    lookup_url_kwarg = 'pk'
...
class UserUpdateProfilePictureView(generics.RetrieveUpdateAPIView):
    permission_classes = [permissions.IsAuthenticated] # <- Habilitado solo para pruebas, luego cambiar a IsAuthenticated
...
class ChangePasswordByIdView(generics.GenericAPIView):
    permission_classes = [permissions.IsAuthenticated]  # <- Habilitado solo para pruebas
...
@api_view(['GET'])
@permission_classes([IsAdminOrTechnician])  # Solo admin o técnicos pueden consultar clientes
...
class AdminUpdateUserView(generics.RetrieveUpdateAPIView):
    permission_classes = [IsAdmin]  # Solo administradores pueden actualizar usuarios
